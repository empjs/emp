...
async function main() {
  ...
  try {
    ...
    for (const packageName of packageNames) {
      await run('corepack', ['pnpm@12.2.1', '--filter', packageName, 'build'])
    }

    const tarballByPackage = new Map()
    for (const packageName of packageNames) {
      const tarballPath = join(packDir, tarballName(packageName))
      await run('corepack', ['pnpm@12.2.1', '--filter', packageName, 'pack', '--out', tarballPath])
    ...
    await run('corepack', ['pnpm@12.2.1', 'install'], {cwd: consumerRoot})

...
var EMP_ADAPTER_VUE = function() {
    var assert = // ../../node_modules/.pnpm/vue-router@3.6.5_vue@2.7.14/node_modules/vue-router/dist/vue-router.mjs
    ...
    var resolveQuery = function resolveQuery(query, extraQuery, _parseQuery) {
    ...
    var createRoute = function createRoute(record, location, redirectedFrom, router) {
    ...
    var formatMatch = function formatMatch(record) {
        var res = [];
        while(record){
            res.unshift(record);
    ...
    var getFullPath = function getFullPath(ref2, _stringifyQuery) {
        var path = ref2.path;
    ...
    var isSameRoute = function isSameRoute(a, b, onlyPath) {
        if (b === START) {
            return a === b;
        } else if (!b) {
            return false;
    ...
    var isIncludedRoute = function isIncludedRoute(current, target) {
        return current.path.replace(trailingSlashRE, "/").indexOf(target.path.replace(trailingSlashRE, "/")) === 0 && (!target.hash || current.hash === target.hash) && queryIncludes(current.query, target.query);
    };
    var queryIncludes = function queryIncludes(current, target) {
    ...
    var handleRouteEntered = function handleRouteEntered(route) {
    ...
    var fillPropsinData = function fillPropsinData(component, data, route, configProps) {
    ...
    var resolveProps = function resolveProps(route, config) {
        switch(typeof config === "undefined" ? "undefined" : _type_of(config)){
            case "undefined":
                return;
            case "object":
                return config;
            ...
            default:
                if (true) {
                    warn(false, 'props in "' + route.path + '" is a ' + (typeof config === "undefined" ? "undefined" : _type_of(config)) + ", expecting an object, function or boolean.");
                }
        }
    };
    var resolvePath = function resolvePath(relative, base, append) {
    ...
    var encodeURIComponentPretty = function encodeURIComponentPretty(str) {
        return encodeURI(str).replace(/[\/?#]/g, function(c) {
            return "%" + c.charCodeAt(0).toString(16).toUpperCase();
        });
    };
    var encodeAsterisk = function encodeAsterisk(str) {
        return encodeURI(str).replace(/[?#]/g, function(c) {
            return "%" + c.charCodeAt(0).toString(16).toUpperCase();
        });
    };
    var tokensToFunction = function tokensToFunction(tokens, options) {
        var matches = new Array(tokens.length);
        for(var i = 0; i < tokens.length; i++){
            if (_type_of(tokens[i]) === "object") {
    ...
    var attachKeys = function attachKeys(re, keys) {
        re.keys = keys;
        return re;
    };
    ...
    var fillParams = function fillParams(path, params, routeMsg) {
        params = params || {};
        try {
    ...
    var normalizeLocation = function normalizeLocation(raw, current, append, router) {
        ...
        if (next._normalized) {
            ...
            if (params && (typeof params === "undefined" ? "undefined" : _type_of(params)) === "object") {
    ...
    var createRouteMap = function createRouteMap(routes, oldPathList, oldPathMap, oldNameMap, parentRoute) {
    ...
    var compileRouteRegex = function compileRouteRegex(path, pathToRegexpOptions) {
    ...
    var normalizePath = function normalizePath(path, parent, strict) {
        if (!strict) {
            path = path.replace(/\/$/, "");
        }
        if (path[0] === "/") {
    ...
    var createMatcher = function createMatcher(routes, router) {
        ...
        function addRoute2(parentOrRoute, route) {
            var parent = (typeof parentOrRoute === "undefined" ? "undefined" : _type_of(parentOrRoute)) !== "object" ? nameMap[parentOrRoute] : void 0;
            createRouteMap([
                route || parentOrRoute
        ...
        function match2(raw, currentRoute, redirectedFrom) {
            ...
            if (name) {
                ...
                if (_type_of(location.params) !== "object") {
                    location.params = {};
                }
                if (currentRoute && _type_of(currentRoute.params) === "object") {
        ...
        function redirect(record, location) {
            ...
            if (!redirect2 || (typeof redirect2 === "undefined" ? "undefined" : _type_of(redirect2)) !== "object") {
                if (true) {
    ...
    var matchRoute = function matchRoute(regex, path, params) {
    ...
    var resolveRecordPath = function resolveRecordPath(path, record) {
        return resolvePath(path, record.parent ? record.parent.path : "/", true);
    };
    var genStateKey = function genStateKey() {
        return Time.now().toFixed(3);
    };
    var getStateKey = function getStateKey() {
        return _key;
    };
    var setStateKey = function setStateKey(key) {
        return _key = key;
    };
    ...
    var handleScroll = function handleScroll(router, to, from, isPop) {
        if (!router.app) {
            return;
        }
    ...
    var getElementPosition = function getElementPosition(el, offset) {
    ...
    var isValidPosition = function isValidPosition(obj) {
        return isNumber(obj.x) || isNumber(obj.y);
    };
    var normalizePosition = function normalizePosition(obj) {
        return {
            x: isNumber(obj.x) ? obj.x : window.pageXOffset,
            y: isNumber(obj.y) ? obj.y : window.pageYOffset
        };
    };
    var normalizeOffset = function normalizeOffset(obj) {
        return {
    ...
    var scrollToPosition = function scrollToPosition(shouldScroll, position) {
        var isObject = (typeof shouldScroll === "undefined" ? "undefined" : _type_of(shouldScroll)) === "object";
        if (isObject && typeof shouldScroll.selector === "string") {
            ...
            if (el) {
                var offset = shouldScroll.offset && _type_of(shouldScroll.offset) === "object" ? shouldScroll.offset : {};
    ...
    var pushState = function pushState(url, replace2) {
        saveScrollPosition();
    ...
    var createNavigationRedirectedError = function createNavigationRedirectedError(from, to) {
        return createRouterError(from, to, NavigationFailureType.redirected, 'Redirected when going from "' + from.fullPath + '" to "' + stringifyRoute(to) + '" via a navigation guard.');
    };
    var createNavigationDuplicatedError = function createNavigationDuplicatedError(from, to) {
    ...
    var createNavigationCancelledError = function createNavigationCancelledError(from, to) {
        return createRouterError(from, to, NavigationFailureType.cancelled, 'Navigation cancelled from "' + from.fullPath + '" to "' + to.fullPath + '" with a new navigation.');
    };
    var createNavigationAbortedError = function createNavigationAbortedError(from, to) {
        return createRouterError(from, to, NavigationFailureType.aborted, 'Navigation aborted from "' + from.fullPath + '" to "' + to.fullPath + '" via a navigation guard.');
    };
    var createRouterError = function createRouterError(from, to, type, message) {
    ...
    var stringifyRoute = function stringifyRoute(to) {
        if (typeof to === "string") {
            return to;
        }
        if ("path" in to) {
    ...
    var isNavigationFailure = function isNavigationFailure(err, errorType) {
        return isError(err) && err._isRouter && (errorType == null || err.type === errorType);
    };
    var runQueue = function runQueue(queue, fn, cb) {
        var step = function step1(index) {
    ...
    var resolveAsyncComponents = function resolveAsyncComponents(matched) {
    ...
    var flatMapComponents = function flatMapComponents(matched, fn) {
    ...
    var isESModule = function isESModule(obj) {
    ...
    var resolveQueue = function resolveQueue(current, next) {
        var i;
    ...
    var extractGuards = function extractGuards(records, name, bind, reverse) {
    ...
    var extractGuard = function extractGuard(def, key) {
        if (typeof def !== "function") {
            def = _Vue.extend(def);
        }
        return def.options[key];
    };
    var extractLeaveGuards = function extractLeaveGuards(deactivated) {
        return extractGuards(deactivated, "beforeRouteLeave", bindGuard, true);
    };
    var extractUpdateHooks = function extractUpdateHooks(updated) {
        return extractGuards(updated, "beforeRouteUpdate", bindGuard);
    };
    var bindGuard = function bindGuard(guard, instance) {
        if (instance) {
    ...
    var extractEnterGuards = function extractEnterGuards(activated) {
        return extractGuards(activated, "beforeRouteEnter", function(guard, _, match2, key) {
            return bindEnterGuard(guard, match2, key);
        });
    };
    var bindEnterGuard = function bindEnterGuard(guard, match2, key) {
    ...
    var getLocation = function getLocation(base) {
    ...
    var checkFallback = function checkFallback(base) {
    ...
    var pushHash = function pushHash(path) {
    ...
    var replaceHash = function replaceHash(path) {
        if (supportsPushState) {
    ...
    var createHref = function createHref(base, fullPath, mode) {
    ...
    var require_vue_runtime_common_dev = __commonJS({
         ...
         */ key: "observeArray",
            ...
            function set2(target2, key, val) {
                ...
                if (isReadonly2(target2)) {
                    warn2('Set operation on key "'.concat(key, '" failed: target is readonly.'));
                    return;
                }
            ...
            function del2(target2, key) {
                if (isUndef(target2) || isPrimitive(target2)) {
                    warn2("Cannot delete reactive property on undefined, null, or primitive value: ".concat(target2));
                }
                if (isArray(target2) && isValidArrayIndex(key)) {
                ...
                if (isReadonly2(target2)) {
                    warn2('Delete operation on key "'.concat(key, '" failed: target is readonly.'));
                    return;
                }
            ...
            function makeReactive(target2, shallow) {
                if (!isReadonly2(target2)) {
                    {
                        if (isArray(target2)) {
                            warn2("Avoid using Array as root value for ".concat(shallow ? "shallowReactive()" : "reactive()", " as it cannot be tracked in watch() or watchEffect(). Use ").concat(shallow ? "shallowRef()" : "ref()", " instead. This is a Vue-2-only limitation."));
                        }
                        var existingOb = target2 && target2.__ob__;
                        if (existingOb && existingOb.shallow !== shallow) {
                            warn2("Target is already a ".concat(existingOb.shallow ? "" : "non-", "shallow reactive object, and cannot be converted to ").concat(shallow ? "" : "non-", "shallow."));
                        }
                    }
                ...
                if (!isPlainObject(target2)) {
                    {
                        if (isArray(target2)) {
                            ...
                            warn2("value cannot be made readonly: ".concat(typeof target2 === "undefined" ? "undefined" : _type_of(target2)));
                        }
                    }
                    return target2;
                }
            ...
            function computed2(getterOrOptions, debugOptions) {
                ...
                var watcher = isServerRendering() ? null : new Watcher(currentInstance, getter, noop2, {
                    lazy: true
                });
            ...
            function doWatch(source, cb) {
                ...
                var warnInvalidSource = function(s) {
                    warn2("Invalid watch source: ".concat(s, ". A watch source can only be a getter/effect function, a ref, a reactive object, or an array of these types."));
                };
         ...
         */ key: "off",
            ...
            function inject2(key, defaultValue) {
                ...
                if (instance) {
                    ...
                    if (provides && key in provides) {
                        ...
                        warn2('injection "'.concat(String(key), '" not found.'));
                    }
                } else {
            ...
            function updateListeners(on, oldOn, add2, remove2, createOnceHandler2, vm) {
                ...
                for(name in on){
                    ...
                    if (isUndef(cur)) {
                        warn2('Invalid handler for event "'.concat(event.name, '": got ') + String(cur), vm);
                    } else if (isUndef(old)) {
                        if (isUndef(cur.fns)) {
            ...
            function bindObjectListeners(data, value) {
                if (value) {
                    if (!isPlainObject(value)) {
                        ...
                        var on = data.on = data.on ? extend2({}, data.on) : {};
            ...
            function bindDynamicKeys(baseObj, values) {
                for(var i = 0; i < values.length; i += 2){
                    ...
                    if (typeof key === "string" && key) {
                        ...
                        warn2("Invalid value for dynamic directive argument (expected string or null): ".concat(key), this);
                    }
                }
                return baseObj;
            }
            ...
            function initSetup(vm) {
                ...
                if (setup) {
                    ...
                    if (isFunction(setupResult)) {
                        ...
                        warn2("setup() should return an object. Received: ".concat(setupResult === null ? "null" : typeof setupResult === "undefined" ? "undefined" : _type_of(setupResult)));
                    }
                }
            }
            ...
            function mergeDefaults2(raw, defaults) {
                ...
                for(var key in defaults){
                    ...
                    if (opt) {
                        ...
                        warn2('props default key "'.concat(key, '" has no corresponding declaration.'));
                    }
                }
                return props2;
            }
            ...
            function renderMixin(Vue3) {
                ...
                Vue3.prototype._render = function() {
                    var vm = this;
                    var _vm_$options = vm.$options, render3 = _vm_$options.render, _parentVnode = _vm_$options._parentVnode;
            ...
            function resolveAsyncComponent(factory, baseCtor) {
                ...
                if (owner && !isDef(factory.owners)) {
                    ...
                    var reject = once2(function(reason) {
                        warn2("Failed to resolve async component: ".concat(String(factory)) + (reason ? "\nReason: ".concat(reason) : ""));
            ...
            function _createElement(context, tag, data, children, normalizationType) {
                if (isDef(data) && isDef(data.__ob__)) {
                    warn2("Avoid using observed data object as vnode data: ".concat(JSON.stringify(data), "\nAlways create fresh vnode data objects in each render!"), context);
                ...
                if (typeof tag === "string") {
                    ...
                    if (config.isReservedTag(tag)) {
                        if (isDef(data) && isDef(data.nativeOn) && data.tag !== "component") {
                            warn2("The .native modifier for v-on is only valid on components but it was used on <".concat(tag, ">."), context);
                        }
            ...
            function createLifeCycle(hookName) {
                return function(fn) {
                    ...
                    if (!target2) {
                        warn2("".concat(formatName(hookName), " is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup()."));
                        return;
                    }
            ...
            var Watcher = /*#__PURE__*/ function() {
                function Watcher(vm, expOrFn, cb, options, isRenderWatcher) {
                    ...
                    if (isFunction(expOrFn)) {
                        ...
                        if (!this.getter) {
                            this.getter = noop2;
                            warn2('Failed watching path: "'.concat(expOrFn, '" Watcher only accepts simple dot-delimited paths. For full control, use a function instead.'), vm);
                        }
                    }
         ...
         */ key: "teardown",
            ...
            var measure;
            {
                var perf = inBrowser2 && window.performance;
                if (perf && // @ts-ignore
                perf.mark && // @ts-ignore
                ...
                for(index = 0; index < queue.length; index++){
                    ...
                    if (has[id] != null) {
                        ...
                        if (circular[id] > MAX_UPDATE_COUNT) {
                            warn2("You may have an infinite update loop " + (watcher.user ? 'in watcher with expression "'.concat(watcher.expression, '"') : "in a component render function."), watcher.vm);
                            break;
                        }
                    }
                }
            ...
            function initProvide(vm) {
                ...
                if (provideOption) {
                    ...
                    var keys = hasSymbol2 ? Reflect.ownKeys(provided) : Object.keys(provided);
                    for(var i = 0; i < keys.length; i++){
                        var key = keys[i];
                    ...
                    Object.keys(result).forEach(function(key) {
                        {
                            defineReactive(vm, key, result[key], function() {
                                warn2('Avoid mutating an injected value directly since the changes will be overwritten whenever the provided component re-renders. injection being mutated: "'.concat(key, '"'), vm);
                            });
                        }
                    });
                    toggleObserving(true);
            ...
            function resolveInject(inject3, vm) {
                if (inject3) {
                    ...
                    var keys = hasSymbol2 ? Reflect.ownKeys(inject3) : Object.keys(inject3);
                    for(var i = 0; i < keys.length; i++){
                        var key = keys[i];
                        ...
                        if (provideKey in vm._provided) {
                            ...
                            warn2('Injection "'.concat(key, '" not found'), vm);
                        }
                    }
                    return result;
                }
            ...
            function createComponent(Ctor, data, context, children, tag) {
                ...
                if (typeof Ctor !== "function") {
                    {
                        warn2("Invalid Component definition: ".concat(String(Ctor)), context);
                    }
                    return;
                }
                var asyncFactory;
            ...
            var strats = config.optionMergeStrategies;
            {
                strats.el = strats.propsData = function(parent, child, vm, key) {
                    if (!vm) {
                        warn2('option "'.concat(key, '" can only be used during instance creation with the `new` keyword.'));
                    }
            ...
            strats.props = strats.methods = strats.inject = strats.computed = function(parentVal, childVal, vm, key) {
                ...
                extend2(ret, parentVal);
                if (childVal) extend2(ret, childVal);
                return ret;
            };
            ...
            function normalizeProps(options, vm) {
                ...
                if (isArray(props2)) {
                    ...
                    warn2('Invalid value for option "props": expected an Array or an Object, but got '.concat(toRawType(props2), "."), vm);
                }
                options.props = res;
            }
            function normalizeInject(options, vm) {
                ...
                if (isArray(inject3)) {
                    ...
                    for(var key in inject3){
                        var val = inject3[key];
                        normalized[key] = isPlainObject(val) ? extend2({
                            from: key
                        }, val) : {
                            from: val
                        };
                    ...
                    warn2('Invalid value for option "inject": expected an Array or an Object, but got '.concat(toRawType(inject3), "."), vm);
                }
            }
            ...
            function assertObjectType(name, value, vm) {
                if (!isPlainObject(value)) {
                    warn2('Invalid value for option "'.concat(name, '": expected an Object, but got ').concat(toRawType(value), "."), vm);
                }
            }
            ...
            var initProxy;
            {
                ...
                var warnNonPresent = function(target2, key) {
                    warn2('Property or method "'.concat(key, '" is not defined on the instance but referenced during render. Make sure that this property is reactive, either in the data option, or for class-based components, by initializing the property. See: https://v2.vuejs.org/v2/guide/reactivity.html#Declaring-Reactive-Properties.'), target2);
                };
                var warnReservedPrefix = function(target2, key) {
                    warn2('Property "'.concat(key, '" must be accessed with "$data.').concat(key, '" because properties starting with "$" or "_" are not proxied in the Vue instance to prevent conflicts with Vue internals. See: https://v2.vuejs.org/v2/api/#data'), target2);
                };
                    ...
                    config.keyCodes = new Proxy(config.keyCodes, {
                        set: function set(target2, key, value) {
                            if (isBuiltInModifier(key)) {
                                warn2("Avoid overwriting built-in modifier in config.keyCodes: .".concat(key));
                                return false;
                            } else {
                                target2[key] = value;
                                return true;
                ...
                var _loop = function(key) {
                    ...
                    var value = validateProp(key, propsOptions, propsData, vm);
                    {
                        ...
                        if (isReservedAttribute(hyphenatedKey) || config.isReservedAttr(hyphenatedKey)) {
                            warn2('"'.concat(hyphenatedKey, '" is a reserved attribute and cannot be used as component prop.'), vm);
                        }
                        defineReactive(props2, key, value, function() {
                            if (!isRoot && !isUpdatingChildComponent) {
                                warn2("Avoid mutating a prop directly since the value will be overwritten whenever the parent component re-renders. Instead, use a data or computed property based on the prop's value. Prop being mutated: \"".concat(key, '"'), vm);
                            }
                        });
                    }
                    if (!(key in vm)) {
                ...
                while(i--){
                    var key = keys[i];
                    {
                        if (methods && hasOwn(methods, key)) {
                            warn2('Method "'.concat(key, '" has already been defined as a data property.'), vm);
                        }
                    }
                    if (props2 && hasOwn(props2, key)) {
                        warn2('The data property "'.concat(key, '" is already declared as a prop. Use prop default value instead.'), vm);
            ...
            function initComputed$1(vm, computed3) {
                ...
                for(var key in computed3){
                    ...
                    if (getter == null) {
                        warn2('Getter is missing for computed property "'.concat(key, '".'), vm);
                    }
                    if (!isSSR) {
                    ...
                    if (!(key in vm)) {
                        ...
                        if (key in vm.$data) {
                            warn2('The computed property "'.concat(key, '" is already defined in data.'), vm);
                        } else if (vm.$options.props && key in vm.$options.props) {
                            warn2('The computed property "'.concat(key, '" is already defined as a prop.'), vm);
                        } else if (vm.$options.methods && key in vm.$options.methods) {
                            warn2('The computed property "'.concat(key, '" is already defined as a method.'), vm);
                        }
                    }
                }
            }
            function defineComputed(target2, key, userDef) {
                ...
                if (sharedPropertyDefinition.set === noop2) {
                    sharedPropertyDefinition.set = function() {
                        warn2('Computed property "'.concat(key, '" was assigned to but it has no setter.'), this);
                    };
                }
                Object.defineProperty(target2, key, sharedPropertyDefinition);
            }
                ...
                for(var key in methods){
                    {
                        if (typeof methods[key] !== "function") {
                            warn2('Method "'.concat(key, '" has type "').concat(_type_of(methods[key]), '" in the component definition. Did you reference the function correctly?'), vm);
                        }
                        if (props2 && hasOwn(props2, key)) {
                            warn2('Method "'.concat(key, '" has already been defined as a prop.'), vm);
                        }
                        if (key in vm && isReserved(key)) {
                            warn2('Method "'.concat(key, '" conflicts with an existing Vue instance method. Avoid defining component methods that start with _ or $.'));
                        }
                    }
            ...
            function registerRef(vnode, isRemoval) {
                ...
                if (_isString || _isRef) {
                    if (isFor) {
                        ...
                        warn2("Invalid template ref type: ".concat(typeof ref3 === "undefined" ? "undefined" : _type_of(ref3)));
                    }
                }
            }
            ...
            function createPatchFunction(backend) {
                ...
                function checkDuplicateKeys(children) {
                    ...
                    for(var i2 = 0; i2 < children.length; i2++){
                        ...
                        if (isDef(key)) {
                            if (seenKeys[key]) {
                                warn2("Duplicate keys detected: '".concat(key, "'. This may cause an update error."), vnode.context);
                            } else {
                                seenKeys[key] = true;
                            }
                        }
            ...
            var raf = inBrowser2 ? window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : setTimeout : /* istanbul ignore next */ /* istanbul ignore next */ function(fn) {
                return fn();
            };
            ...
            function checkDuration(val, name, vnode) {
                if (typeof val !== "number") {
                    warn2("<transition> explicit ".concat(name, " duration is not a valid number - got ").concat(JSON.stringify(val), "."), vnode.context);
                } else if (isNaN(val)) {
                    warn2("<transition> explicit ".concat(name, " duration is NaN - the duration expression might be incorrect."), vnode.context);
                }
            }
            ...
            function actuallySetSelected(el, binding, vm) {
                ...
                if (isMultiple && !Array.isArray(value)) {
                    warn2('<select multiple v-model="'.concat(binding.expression, '"> expects an Array value for its binding, but got ').concat(Object.prototype.toString.call(value).slice(8, -1)), vm);
                    return;
                }
                var selected, option;
                    ...
                    for(var i = 0; i < rawChildren.length; i++){
                        ...
                        if (c.tag) {
                            if (c.key != null && String(c.key).indexOf("__vlist") !== 0) {
                                ...
                                warn2("<transition-group> children must be keyed: <".concat(name, ">"));
                            }
                        }
                    }
                    if (prevChildren) {
    ...
    var castQueryParamValue = function castQueryParamValue(value) {
        return value == null || (typeof value === "undefined" ? "undefined" : _type_of(value)) === "object" ? value : String(value);
    };
    var trailingSlashRE = /\/?$/;
    ...
    function isObjectEqual(a, b) {
        ...
        return aKeys.every(function(key, i) {
            ...
            if ((typeof aVal === "undefined" ? "undefined" : _type_of(aVal)) === "object" && (typeof bVal === "undefined" ? "undefined" : _type_of(bVal)) === "object") {
            ...
            if (this.tag === "a") {
                ...
                if (a) {
                    ...
                    for(var event in aData.on){
                        ...
                        if (event in on) {
                            aData.on[event] = Array.isArray(handler$1) ? handler$1 : [
                                handler$1
                            ];
                        }
                    }
    ...
    var hasSymbol = typeof Symbol === "function" && _type_of(Symbol.toStringTag) === "symbol";
            ...
            try {
                hook(route, current, function(to) {
                    if (to === false) {
                        ...
                        this$1$1.ensureURL(true);
                        abort(to);
                    } else if (typeof to === "string" || (typeof to === "undefined" ? "undefined" : _type_of(to)) === "object" && (typeof to.path === "string" || typeof to.name === "string")) {
                        abort(createNavigationRedirectedError(current, route));
                        if ((typeof to === "undefined" ? "undefined" : _type_of(to)) === "object" && to.replace) {
                            this$1$1.replace(to);
                        } else {
                            this$1$1.push(to);
                        }
    ...
    var VueRouter = function VueRouter2(options) {
        ...
        if (true) {
            warn(_instanceof(this, VueRouter2), "Router must be called with the new operator.");
        }
        this.app = null;
        this.apps = [];
        this.options = options;
    ...
    VueRouter.prototype.init = function init(app) {
        ...
        if (_instanceof(history, HTML5History) || _instanceof(history, HashHistory)) {
            ...
            var setupListeners2 = function setupListeners2(routeOrError) {
...
//# sourceMappingURL=vueRouter.development.umd.js.map
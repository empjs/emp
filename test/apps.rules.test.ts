...
describe('legacy apps rules coverage', () => {
  ...
  test('apps acceptance keeps browser smoke out of the default apps acceptance lane', async () => {
    ...
    expect(rootTestTargets).toContain('test/apps/browser/vue-3-project/remote.browser.ts')
    ...
    expect(browserRunner).toContain("'watch', '--browser', '--browser.headless=false'")
    expect(browserRunner).toContain("'corepack'")
    expect(browserRunner).toContain('test/apps/browser/**/*.browser.ts')
    expect(workflowGuard).toContain('ROOT_BROWSER_TEST_TARGETS')
    expect(workflowGuard).toContain('test/apps/browser')
    expect(workflowGuard).toContain('test:apps:browser')
    expect(rootPackage.devDependencies?.['@rstest/browser']).toBe('0.10.6')

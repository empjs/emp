...
describe('toolchain version contract', () => {
  ...
  test('Rstest uses the current Rsbuild and Rspack stack', () => {
    ...
    expect(pkg.devDependencies['@rstest/core']).toBe('0.11.4')
    expect(pkg.devDependencies['@rstest/browser']).toBe('0.11.4')
    expect(cliPkg.devDependencies['@rstest/core']).toBe('0.11.4')
  ...
  test('cli depends on the current Rspack core and dev-server stack with TS7-aware tooling', () => {
    ...
    expect(cliPkg.dependencies['@rsdoctor/rspack-plugin']).toBe('1.6.1')
    ...
    expect(cliPkg.dependencies['ts-checker-rspack-plugin']).toBe('^1.6.0')
    expect(cliPkg.devDependencies['@vue/tsconfig']).toBe('^0.9.1')
    expect(lockfile).toContain('@rsdoctor/rspack-plugin@1.6.1')
    expect(lockfile).toContain('@rspack/dev-server@2.2.0')
    expect(lockfile).toContain('ts-checker-rspack-plugin@1.6.0')
  ...
  test('emp-share keeps Module Federation current and SWC patched', () => {
    ...
    expect(sharePkg.dependencies['@module-federation/rspack']).toBe('^2.8.2')
    expect(sharePkg.dependencies['@module-federation/runtime']).toBe('^2.8.2')
    expect(sharePkg.dependencies['@module-federation/runtime-tools']).toBe('^2.8.2')
    expect(sharePkg.dependencies['@module-federation/sdk']).toBe('^2.8.2')
    expect(sharePkg.devDependencies['@swc/core']).toBe('^1.15.47')
    expect(lockfile).toContain('@module-federation/rspack@2.8.2')
    expect(lockfile).toContain('@module-federation/dts-plugin@2.8.2')
    expect(lockfile).not.toContain('@module-federation/rspack@2.8.1')
    expect(lockfile).not.toContain('@module-federation/dts-plugin@2.8.1')

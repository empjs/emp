...
describe('toolchain version contract', () => {
  test('root README stays aligned with the v4 homepage product surface', () => {
    ...
    expect(readme).toContain('明确的产物格式')
    expect(readme).toContain('build.format')
    expect(readme).toContain('build.targets')
  ...
  test('Rstest uses the current Rsbuild and Rspack stack', () => {
    ...
    expect(pkg.devDependencies['@rstest/core']).toBe('0.11.12')
    expect(pkg.devDependencies['@rstest/browser']).toBe('0.11.12')
    expect(cliPkg.devDependencies['@rstest/core']).toBe('0.11.12')
    expect(postcssPkg.dependencies['postcss-loader']).toBe('^8.2.1')
    expect(lockfile).toContain('@rstest/core@0.11.12')
    expect(lockfile).toContain('@rstest/browser@0.11.12')
    expect(lockfile).toContain('@rsbuild/core@2.2.3')
    expect(lockfile).toContain('@rspack/core@2.2.3')
    ...
    expect(lockfile).not.toContain("'@rstest/browser@0.11.1':")
    expect(lockfile).not.toContain("'@rstest/core@0.11.4':")
    expect(lockfile).not.toContain("'@rstest/browser@0.11.4':")
    expect(lockfile).not.toContain("'@rsbuild/core@2.1.8':")
    expect(lockfile).not.toContain("'@rsbuild/core@2.1.11':")
    expect(lockfile).not.toContain("'@rspack/core@2.1.8':")
    expect(lockfile).not.toContain("'@rspack/core@2.1.9':")
  ...
  test('cli depends on the current Rspack core and dev-server stack with TS7-aware tooling', () => {
    ...
    expect(cliPkg.dependencies['@rspack/core']).toBe('2.2.3')
    expect(cliPkg.dependencies['@rspack/dev-server']).toBe('^2.2.1')
    expect(reactPkg.dependencies['@rspack/plugin-react-refresh']).toBe('^2.0.2')
    expect(cliPkg.dependencies['@swc/helpers']).toBe('^0.5.23')
    expect(cliPkg.dependencies['@rsdoctor/rspack-plugin']).toBe('1.6.3')
    expect(cliPkg.dependencies.cors).toBe('2.8.6')
    expect(cliPkg.dependencies['fs-extra']).toBe('11.4.0')
    expect(cliPkg.dependencies['html-webpack-plugin']).toBe('5.6.8')
    expect(cliPkg.dependencies['ts-checker-rspack-plugin']).toBe('^1.6.1')
    expect(cliPkg.devDependencies['@vue/tsconfig']).toBe('^0.9.1')
    expect(lockfile).toContain('@rsdoctor/rspack-plugin@1.6.3')
    expect(lockfile).toContain('@rspack/dev-server@2.2.1')
    expect(lockfile).toContain('ts-checker-rspack-plugin@1.6.1')
    expect(lockfile).not.toContain('@rsdoctor/rspack-plugin@1.5.18')
  ...
  test('emp-share keeps Module Federation current and SWC patched', () => {
    ...
    expect(sharePkg.dependencies['@module-federation/rspack']).toBe('^2.9.0')
    expect(sharePkg.dependencies['@module-federation/runtime']).toBe('^2.9.0')
    expect(sharePkg.dependencies['@module-federation/runtime-tools']).toBe('^2.9.0')
    expect(sharePkg.dependencies['@module-federation/sdk']).toBe('^2.9.0')
    expect(sharePkg.devDependencies['@swc/core']).toBe('^1.16.1')
    expect(lockfile).toContain('@module-federation/rspack@2.9.0')
    expect(lockfile).toContain('@module-federation/dts-plugin@2.9.0')
    expect(lockfile).not.toContain('@module-federation/rspack@2.8.2')
    expect(lockfile).not.toContain('@module-federation/dts-plugin@2.8.2')
    expect(lockfile).toContain('@swc/core@1.16.1')
    expect(lockfile).not.toContain('@swc/core@1.15.47')
    expect(shareLockImporter).toContain('specifier: ^1.16.1')
    expect(shareLockImporter).toContain('version: 1.16.1(@swc/helpers@0.5.23)')
  })
  ...
  test('Rslib stable declaration tooling keeps native TS7 support and migrated externals', () => {
    const pkg = readJson('package.json')
    const rslibPresetsPkg = readJson('packages/rslib-presets/package.json')
    const workspace = readText('pnpm-workspace.yaml')
    const lockfile = readText('pnpm-lock.yaml')
    expect(pkg.devDependencies['@rslib/core']).toBe('1.0.0')
    expect(rslibPresetsPkg.dependencies['@rslib/core']).toBe('1.0.0')
    ...
    expect(lockfile).toContain('rsbuild-plugin-dts@1.0.0(')
    expect(lockfile).not.toContain('rsbuild-plugin-dts@1.0.0-rc.2')
    expect(lockfile).not.toContain('rsbuild-plugin-dts@1.0.0-beta.2')
    expect(lockfile).not.toContain("'@rslib/core@1.0.0-beta.2':")

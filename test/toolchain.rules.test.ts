...
describe('toolchain version contract', () => {
  ...
  test('Rstest uses the current Rsbuild and Rspack stack', () => {
    ...
    expect(pkg.devDependencies['@rstest/core']).toBe('0.11.3')
    expect(pkg.devDependencies['@rstest/browser']).toBe('0.11.3')
    expect(cliPkg.devDependencies['@rstest/core']).toBe('0.11.3')
    expect(postcssPkg.dependencies['postcss-loader']).toBe('^8.2.1')
    expect(lockfile).toContain('@rstest/core@0.11.3')
    expect(lockfile).toContain('@rstest/browser@0.11.3')
    expect(lockfile).toContain('@rsbuild/core@2.1.7')

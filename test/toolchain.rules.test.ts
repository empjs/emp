...
describe('toolchain version contract', () => {
  ...
  test('Rslib RC declaration tooling keeps native TS7 support and migrated externals', () => {
    const pkg = readJson('package.json')
    const rslibPresetsPkg = readJson('packages/rslib-presets/package.json')
    const workspace = readText('pnpm-workspace.yaml')
    const lockfile = readText('pnpm-lock.yaml')
    expect(pkg.devDependencies['@rslib/core']).toBe('1.0.0-rc.2')
    expect(rslibPresetsPkg.dependencies['@rslib/core']).toBe('1.0.0-rc.2')
    ...
    expect(lockfile).toContain('rsbuild-plugin-dts@1.0.0-rc.2')

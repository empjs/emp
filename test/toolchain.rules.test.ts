...
describe('toolchain version contract', () => {
  ...
  test('Rstest uses the current Rsbuild and Rspack stack', () => {
    ...
    expect(pkg.devDependencies['@rstest/core']).toBe('0.11.11')
    expect(pkg.devDependencies['@rstest/browser']).toBe('0.11.11')
    expect(cliPkg.devDependencies['@rstest/core']).toBe('0.11.11')
    expect(postcssPkg.dependencies['postcss-loader']).toBe('^8.2.1')
    expect(lockfile).toContain('@rstest/core@0.11.11')
    expect(lockfile).toContain('@rstest/browser@0.11.11')
    expect(lockfile).toContain('@rsbuild/core@2.2.1')
    expect(lockfile).toContain('@rspack/core@2.2.1')
  ...
  test('cli depends on the current Rspack core and dev-server stack with TS7-aware tooling', () => {
    ...
    expect(cliPkg.dependencies['@rspack/core']).toBe('2.2.1')

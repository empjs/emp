...
describe('release rules', () => {
  ...
  test('trusted publisher workflow uses publish.yml and derived prerelease tag automation', async () => {
    ...
    expect(workflow).toMatch(/confirm_publish:[\s\S]*?exact package\.json version/)
    ...
    expect(workflow).toMatch(/INPUT_PACKAGE:\s*\$\{\{ inputs\.package \|\| '' \}\}/)
    expect(workflow).toMatch(/INPUT_CONFIRM_PUBLISH:\s*\$\{\{ inputs\.confirm_publish \|\| '' \}\}/)
    expect(workflow).toMatch(/FORCE_ALL=\$INPUT_FORCE_ALL/)
    expect(workflow).toMatch(/Tag version .* does not match/)

...
describe('release acceptance report', () => {
  test('builds the release credential model from current root and browser test targets', async () => {
    ...
    expect(model.brandLogo?.sourcePath).toBe('docs/assets/emp-v4-logo.png')
    expect(model.brandLogo?.dataUri).toContain('data:image/png;base64,')
  })
  ...
  test('renders a self-contained Chinese HTML credential with command status and coverage sections', async () => {
    ...
    expect(html).toContain('class="dashboard-card chart-card"')
    ...
    expect(html).toContain('alt="EMP v4"')
    ...
    expect(html).toContain('构建号')
    expect(html).toContain('验收时间')
    expect(html).toContain('命令通过率')
    expect(html).toContain('发布门禁')
    expect(html).toContain('验证命令')
    expect(html).toContain('产物证据')
